...
router.post('/approvePengajuan', async (req, res) => {
  ...
  try {
    ...
    const [program] = await new Promise((resolve, reject) => {
      db.query(`SELECT jumlah, jumlah_sisa FROM kegiatan_csr WHERE id = ?`, [pengajuan.kegiatan_id], (err, rows) => err ? reject(err) : resolve(rows));
    });
...
router.post("/uploadEviden", upload.single("file"), (req, res) => {
  try {
    ...
    const sql = `
      INSERT INTO bukti_dukung (kegiatan_mitra_id, file_name, keterangan) 
      VALUES (?, ?, ?)
    `;
...
router.post("/selesaikanProgram", (req, res) => {
  ...
  // Hanya update status di kegiatan_csr
  const sql = `

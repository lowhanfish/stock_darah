  ...
  <span class="h_fotter_title">Copyright : Diskominfo & DKPSDM Kab. Konawe Selatan</span>
        ...
        <div class="putihTrans formLogin shadow-5">
          ...
          <form @submit.prevent="btn_login()">
            <!-- Username -->
            <q-input v-model="user.username" outlined rounded :dense="true" color="red-10" bg-color="white"
              placeholder="Username" class="q-mb-md">
              <template v-slot:prepend>
            ...
            <!-- Password -->
            <q-input v-model="user.password" outlined rounded :dense="true" color="red-10" bg-color="white"
              placeholder="Password" :type="showPassword ? 'text' : 'password'" class="q-mb-md">
              <template v-slot:prepend>
            ...
            <!-- Tombol Login -->
            <q-btn type="submit" push color="red-7" text-color="white" label="LOGIN" class="full-width q-mt-sm"
              unelevated rounded>
          ...
          <div class="transDarah">
            <span class="h_fotter_titleform">Copyright : BLUD RS Konawe Utara</span>
          </div>
        </div>
      </div>
    </div>
    <audio ref="loginSound" src="../../../public/sound/notif.mp3" preload="auto"></audio>
  </div>
...
export default {
  ...
  methods: {
      ...
      if (this.validUser()) {
        ...
        fetch(this.url.LOGIN_URL, {
          ...
          .then((result) => {
            ...
            // 🔊 UNLOCK AUDIO (INI KUNCINYA)
            this.unlockAudioAfterLogin()
    ...
    unlockAudioAfterLogin() {
      const audio = this.$refs.loginSound
      if (!audio) return
      ...
      audio.play().then(() => {
        ...
        console.log('🔓 Audio unlocked (DOM-based)')
      }).catch(err => {
